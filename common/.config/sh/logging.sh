...
#
# Colours resolve once at source time rather than per call, since the copies
...
# Prefixes are the level marker of last resort (see the colour note above), so
# every level needs its own.
__log_message() {
  __log_prefix=$1
  __log_colour=$2
  shift 2
  printf '%s%s %s%s\n' "$__log_colour" "$__log_prefix" "$*" "$_LOG_CLEAR" >&2
}
