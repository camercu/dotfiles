...
if [[ -t 2 ]] && is-installed tput; then
  ...
  typeset -g _LOG_GRAY=$(tput setaf 8)
  ...
  typeset -g _LOG_BLUE='' _LOG_GRAY='' _LOG_YELLOW='' _LOG_RED='' _LOG_GREEN='' _LOG_CLEAR=''
fi

function info    { print -- "${_LOG_BLUE}[*] $*${_LOG_CLEAR}"   >&2; }
function debug   { print -- "${_LOG_GRAY}[~] $*${_LOG_CLEAR}"   >&2; }
